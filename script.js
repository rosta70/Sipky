// script.js - CELÝ SOUBOR (BEZ JAKÉHOKOLIV AUDIA A TTS)
...
// --- PŮVODNÍ AUDIO A TTS FUNKCE BYLY ODEBRÁNY ---
...
    // KONTROLA DAT: Zajištění, že se načítá platný stav
...
    alert(`Rozehraná hra (${gameValue}x01) byla úspěšně načtena! Na řadě je ${players[currentPlayerIndex].name}`);
    
    localStorage.removeItem(SAVED_GAME_KEY); 
    checkSavedGame();
}
...
function addPlayer() {
    // KLÍČOVÁ OPRAVA: Povolit přidávání, jen pokud hra NEZAČALA
...
    history = []; 
    
    alert(`Hra ${value}x01 spuštěna. Začíná ${players[currentPlayerIndex].name}`);
...
    
    // Původní volání audio bylo odebráno
    
    if (currentMultiplier === 2) {
...
// --- FUNKCE END ROUND (NYNÍ SYNCHRONNÍ) ---
function endRound() {
    const player = players[currentPlayerIndex];
...
        alert(`${player.name} hodil ${totalScore} bodů. Zbývá: ${newScore}`); 
    }
    
    if (winner) {
...
    // Původní asynchronní audio sekvence a volání funkce finalizeRoundLogic bylo nahrazeno synchronní logikou
    finalizeRoundLogic(player, scoreBeforeRound, totalScore, currentThrows, newScore, gameJustEnded);
    
    // Společná funkce pro finální logiku kola (která je nyní synchronní)
...
        currentThrowIndex = 0;
        
        // KLÍČOVÝ KROK: PŘEPNUTÍ HRÁČE ZDE
        if (gameStarted) {
            currentPlayerIndex = (currentPlayerIndex + 1) % players.length;
            
            alert(`Na řadě je ${players[currentPlayerIndex].name}`);
        }
        
        // Uložení stavu A VYKRESLENÍ se správným, PŘEPNUTÝM INDEXEM
        saveState(); 
        renderPlayers();
        updateInputDisplay();
...
        if (endGameBtn) endGameBtn.style.display = 'inline-block';
        
        alert(`Vráceno. Na řadě je ${players[currentPlayerIndex].name}`);
    } else {
