// script.js - CELÝ SOUBOR (FINÁLNÍ VERZE: OPRAVENÉ SYNCHRONNÍ PŘEPÍNÁNÍ HRÁČŮ A AUDIO)
...
function endRound() {
    ...
    // --- AUDIO SEKVENČNÍ LOGIKA (Asynchronní) ---
    ...
    // Spuštění kompletní audio sekvence a navázání finální logiky
    ...
    // Společná funkce pro finální logiku kola (která je nyní synchronní s dokončením audio)
    function finalizeRoundLogic(player, scoreBeforeRound, totalScore, currentThrows, newScore, gameJustEnded) {
        ...
        if (gameStarted) {
            ...
            // TTS: Oznámení dalšího hráče
            const tts = new SpeechSynthesisUtterance(`Na řadě je ${players[currentPlayerIndex].name}`);
            tts.lang = 'cs-CZ';
            window.speechSynthesis.speak(tts);
        }
...
function undoLastThrow() {
    ...
    if (gameStarted) {
        ...
        const tts = new SpeechSynthesisUtterance(`Vráceno. Na řadě je ${players[currentPlayerIndex].name}`);
        tts.lang = 'cs-CZ';
        window.speechSynthesis.speak(tts);
    } else {
        setupButtons.forEach(btn => btn.disabled = false);
        if (historyButton) historyovaná data
// ... (ostatní funkce beze změny)

// --- FUNKCE PRO ZAMÍCHÁNÍ POŘADÍ HRÁČŮ ---

function shufflePlayers() {
    // ... (Logika zamíchání beze změny)
    let currentIndex = players.length;
    let randomIndex;
...
function endRound() {
    ...
    if (newScore === 0) {
        ...
        // Standardní odečet
        // KLÍČOVÝ KROK: Skóre odečítáme zde!
        player.score = newScore; 
    }
